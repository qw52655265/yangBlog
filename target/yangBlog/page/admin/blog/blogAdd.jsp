											...
											<c:choose>
												...
												<c:otherwise>
													<c:forEach items="${types }" var="c">
														<option value="${c.itemCode }">${c.itemName }</option>
													</c:forEach>
												</c:otherwise>
											</c:choose>
										</select>
...
<!-- 							</tr> -->
							...
							<tr>
								<td>内容摘要：</td>
								<td><input id="summaryInput" type="text" class="form-control" placeholder="如不填写则自动截取内容描述的前70个字符！"/></td>
							</tr>
							<tr>
								<td>内容描述：<span class="required">*</span></td>
								<td>
									<!-- 加载编辑器的容器 -->
								    <script id="myEditor" name="content" type="text/plain" style="width:100%;height:340px;"></script>
					    		</td>
							</tr>
						</table>
					</div>
				...
				<div style="text-align: center;">
					<a href="javascript:void(0)" id="saveAddBtn" class="btn btn-success">保存提交</a>
				</div>
				<form action="" method="post" id="addBlogForm" autocomplete="off">
